      ...
      return (
        ...
        >
          <div className={`md:w-1/2 mb-6 md:mb-0 ${reverse ? "md:pl-16" : "md:pr-16 md:text-right"}`}>
            ...
            <h4 className="font-heading text-2xl font-bold text-white mb-4 normal-case">{exp.titulo}</h4>
            <p className={`text-white/60 text-sm leading-relaxed max-w-md ${reverse ? "" : "md:ml-auto"}`}>
              {exp.descripcion}
            </p>
          </div>
            ...
            <div className={`glass p-6 rounded-2xl group-hover:border-cyan-500/30 transition-all ${reverse ? "md:text-right" : ""}`}>
              <div className={`flex flex-wrap gap-2 ${reverse ? "justify-start md:justify-end" : ""}`}>
                {exp.tecnologias.split(',').map((tech, i) => (
                  <span key={i} className="px-2 py-1 bg-white/5 rounded text-[10px] font-mono text-white/50 border border-white/5 uppercase">
                    {tech.trim()}
                  </span>
                ))}
              </div>
              <div className="flex gap-4 mt-4 justify-start md:justify-end">
                <a href={exp.enlaceGitHub} target="_blank" rel="noreferrer" className="text-white/40 hover:text-cyan-400 transition-colors">
                  <IconArrowUpRight size={18} />
                </a>
                <a href={exp.enlaceWeb} target="_blank" rel="noreferrer" className="text-white/40 hover:text-cyan-400 transition-colors">
